...
def run_cli(node,args):
    ...
    log(f"{node}: ejecutando {' '.join(args)}")
...
def wait_for_mempool(active):
    ...
    while True:
        ...
        log(f"Mempool: {size}/{dynamic_limit}")

        if size < dynamic_limit:

            return

        log("Mempool alto, esperando...")
...
def redistribute_from_miners(active):
    ...
    rich_miners = [
        ...
        n for n,b in balances.items()
        if n in MINER_NODES and b > avg_balance * 2
    ...
    for sender in active:
        ...
        for receiver in active:
            ...
            if txid:
                ...
                tx_time = round(time.time()-tx_start,2)

                log(f"TX {txid} ({tx_time}s)")
...
def main():
    ...
    while True:
        ...
        elapsed_cycle = round(time.time()-cycle_start,2)

        log(f"Tiempo ciclo: {elapsed_cycle}s")
