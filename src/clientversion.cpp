 ...
 * for both jigocoind and jigocoin-qt, to make it harder for attackers to
 * target servers or GUI users specifically.
 */
const std::string UA_NAME("Jigocoin");


#include <jigocoin-build-info.h>
// The <jigocoin-build-info.h>, which is generated by the build environment (cmake/script/GenerateBuildInfo.cmake),
...
/**
 * Format the subversion field according to BIP 14 spec (https://github.com/jigocoin/bips/blob/master/bip-0014.mediawiki)
 */
...
{
    const auto copyright_devs =
        strprintf(_(COPYRIGHT_HOLDERS), COPYRIGHT_HOLDERS_SUBSTITUTION).translated;

    std::string strCopyrightHolders = strPrefix + copyright_devs;

    // Preserve upstream attribution
    if (copyright_devs.find("Jigocoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix +
            "Portions Copyright (C) 2009-2026 The Jigocoin Core developers";
    }
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ancrumar/jigocoin-core>";
