                     ...
                     TxVerbosity verbosity = TxVerbosity::SHOW_DETAILS)
{
    ...
    // Call into TxToUniv() in jigocoin-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in jigocoin-common, so we query them here and push the
...
static RPCMethod decodepsbt()
{
    return RPCMethod{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed Jigocoin transaction.",
                {
...
static RPCMethod combinepsbt()
{
    return RPCMethod{
        "combinepsbt",
        "Combine multiple partially signed Jigocoin transactions into one transaction.\n"
