...
//! Specify whether parameter should be parsed by jigocoin-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make jigocoin-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `jigocoin-cli` attempt to parse the
 ...
 * `jigocoin-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make jigocoin-cli treat them as positional parameters when `-named` is used.
 * This prevents `jigocoin-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so jigocoin-cli
 ...
 *
 * For example, the command `jigocoin-cli -named createwallet "my=wallet"`,
