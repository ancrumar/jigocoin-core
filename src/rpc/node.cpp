...
static RPCMethod echo(const std::string& name)
{
    return RPCMethod{
        ...
        "Simply echo back the input arguments. This command is for testing.\n"
                ...
                "jigocoin-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCMethod{
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new jigocoin-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new jigocoin-echo executable,
                // and spawn jigocoin-echo below instead of jigocoin-node. But
                // using jigocoin-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("jigocoin-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a jigocoind
                // process not a jigocoind-node process, so just create a local
