...
static void SetupJigocoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " jigocoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The jigocoin-tx tool is used for creating and modifying jigocoin transactions.\n\n"
                "jigocoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded jigocoin transaction, or with \"-create [commands]\" to create a hex-encoded jigocoin transaction.\n"
                "\n"
                "Usage: jigocoin-tx [options] <hex-tx> [commands]\n"
                "or:    jigocoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded jigocoin transaction
