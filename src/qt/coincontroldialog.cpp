...
{
    GUIUtil::setClipboard(JigocoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    JigocoinUnit nDisplayUnit = JigocoinUnit::BTC;
    ...
    l2->setText(JigocoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(JigocoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(JigocoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(JigocoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    QString toolTip4 = tr("Can vary +/- %1 kano(s) per input.").arg(dFeeVary);
...
void CoinControlDialog::updateView()
{
    ...
    JigocoinUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show jigocoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, JigocoinUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, JigocoinUnits::format(nDisplayUnit, nSum));
