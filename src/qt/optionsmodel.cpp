...
/** Call node.updateRwSetting() with Jigocoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Jigocoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/jigocoin/jigocoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because jigocoin 22.x will not try to read these.
...
//! Convert enabled/size values to jigocoin -prune setting.
...
//! Get pruning enabled value to show in GUI from jigocoin -prune setting.
...
//! Get pruning size value to show in GUI from jigocoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayJigocoinUnit")) {
        settings.setValue("DisplayJigocoinUnit", QVariant::fromValue(JigocoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayJigocoinUnit");
    if (unit.canConvert<JigocoinUnit>()) {
        m_display_jigocoin_unit = unit.value<JigocoinUnit>();
    } else {
        m_display_jigocoin_unit = JigocoinUnit::BTC;
        settings.setValue("DisplayJigocoinUnit", QVariant::fromValue(m_display_jigocoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if jigocoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_jigocoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<JigocoinUnit>() == m_display_jigocoin_unit) return;
    m_display_jigocoin_unit = new_unit.value<JigocoinUnit>();
    QSettings settings;
    settings.setValue("DisplayJigocoinUnit", QVariant::fromValue(m_display_jigocoin_unit));
    Q_EMIT displayUnitChanged(m_display_jigocoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/jigocoin/jigocoin/pull/8273
    ...
    // (https://github.com/jigocoin-core/gui/issues/567).
