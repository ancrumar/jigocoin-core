...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<JigocoinUnit>("JigocoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. jigocoin_de.qm (shortcut "de" needs to be defined in jigocoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. jigocoin_de_DE.qm (shortcut "de_DE" needs to be defined in jigocoin.qrc)
...
static const char* qt_argv = "jigocoin-qt";

JigocoinApplication::JigocoinApplication()
...
void JigocoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the JigocoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", JigocoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void JigocoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool JigocoinApplication::createOptionsModel(bool resetSettings)
{
...
void JigocoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new JigocoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &JigocoinGUI::quitRequested, this, &JigocoinApplication::requestShutdown);
...
void JigocoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void JigocoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool JigocoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void JigocoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &JigocoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &JigocoinApplication::handleRunawayException);
    connect(this, &JigocoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &JigocoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void JigocoinApplication::parameterSetup()
{
...
void JigocoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void JigocoinApplication::requestInitialize()
{
...
void JigocoinApplication::requestShutdown()
{
...
void JigocoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    // jigocoin: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &JigocoinGUI::handlePaymentRequest);
        connect(window, &JigocoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void JigocoinApplication::handleRunawayException(const QString &message)
{
...
void JigocoinApplication::handleNonFatalException(const QString& message)
{
...
WId JigocoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool JigocoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", JigocoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(jigocoin);
    Q_INIT_RESOURCE(jigocoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(JIGOCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see jigocoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see jigocoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse jigocoin.conf, determine network, switch to network specific
    ...
    // jigocoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
