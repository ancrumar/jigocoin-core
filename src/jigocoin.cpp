// Copyright (c) 2025-present The Jigocoin Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries jigocoin-node, jigocoin-gui.
  -M, --monolithic       Run monolithic binaries jigocoind, jigocoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'jigocoin-qt [ARGS]' or 'jigocoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'jigocoind [ARGS]' or 'jigocoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'jigocoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'jigocoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'jigocoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_jigocoin [ARGS]'.
  chainstate [ARGS] Run jigocoin kernel chainstate util, equivalent to running 'jigocoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_jigocoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_jigocoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "jigocoin-gui" : "jigocoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "jigocoin-node" : "jigocoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("jigocoin-cli");
            // Since "jigocoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("jigocoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("jigocoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_jigocoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("jigocoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_jigocoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_jigocoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("jigocoin-util");
        } else {
...
//! Execute the specified jigocoind, jigocoin-qt or other command line in `args`
...
//!             be a relative path to a jigocoind, jigocoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/jigocoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/jigocoin/jigocoin/pull/31375#discussion_r1861814807)
