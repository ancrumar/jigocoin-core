...
namespace init {
namespace {
const char* EXE_NAME = "jigocoin-node";

class JigocoinNodeInit : public interfaces::Init
{
public:
    JigocoinNodeInit(node::NodeContext& node, const char* arg0)
        : m_node(node),
...
{
    auto init = std::make_unique<init::JigocoinNodeInit>(node, argc > 0 ? argv[0] : "");
    // Check if jigocoin-node is being invoked as an IPC server. If so, then
