...
thread_local ThreadContext g_thread_context; // NOLINT(jigocoin-nontrivial-threadlocal)
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking jigocoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before jigocoin
    // code in the destructors are run. This way if the jigocoin code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In jigocoin core, running these callbacks in the right order is
