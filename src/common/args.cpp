...
const char * const JIGOCOIN_CONF_FILENAME = "jigocoin.conf";
const char * const JIGOCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //jigocoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", JIGOCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Jigocoin
    //   new: C:\Users\Username\AppData\Local\Jigocoin
    // macOS: ~/Library/Application Support/Jigocoin
    // Unix-like: ~/.jigocoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Jigocoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Jigocoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Jigocoin";
#else
    // Unix-like
    return pathRet / ".jigocoin";
#endif
#endif
}
